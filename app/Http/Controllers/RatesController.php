...
class RatesController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $rules = [
            ...
            'updated_rate' => 'nullable|numeric|min:0|max:999999999',
        ];
        ...
        if ($data['type'] === 'organization_rates') {
            $updated_rate = $data['updated_rate'];
            // Update the rate in the `user_org_subscriptions` table
            auth()->user()->subscriptions()->where('user_org_subscriptions.id', $data['id'])->update([
                'rate' => $updated_rate,
        ...
        if ($data['type'] === 'categories_without_rates') {
            if ($data['updated_rate'] == NULL) {
                return redirect()->back()->with('message', ['error', 'You must enter a rate.']);
            } else {
